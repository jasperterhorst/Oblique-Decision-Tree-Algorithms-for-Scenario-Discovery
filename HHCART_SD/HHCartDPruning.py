...
class HHCartDPruningClassifier(BaseEstimator, ClassifierMixin):
    ...
    Args:
        ...
        force_oblique (bool): If True, forces oblique splits even if axis-aligned splits are available.
    ...
    def fit(self, X: Union[np.ndarray, pd.DataFrame], y: Union[np.ndarray, pd.Series]):
        ...
        for d in range(full_depth + 1):
            ...
            try:
                ...
                # Count number of axis aligned splits, and add to df
                axis_count = sum(
                    isinstance(n, DecisionNode) and n.is_axis_aligned
                    for n in pruned_tree.root.traverse_yield()
                )
                metrics["axis_aligned_splits"] = axis_count
    ...
    def _build_full_tree(self, X: np.ndarray, y: np.ndarray, depth: int, progress_bar=None) -> DecisionTree:
        ...
        def build(X, y, depth) -> TreeNode:
            ...
            is_axis_aligned = (rule == rule_identity)
            ...
            if is_axis_aligned and self.debug:
                nonzero = np.flatnonzero(weights)
                if len(nonzero) != 1 or not np.isclose(np.abs(weights[nonzero[0]]), 1.0):
                    print(f"[DEBUG/WARNING] Axis-aligned split expected one-hot, but got weights={weights}")
                else:
            ...
            if self.debug:
                print(
                    f"[SPLIT] node_id={current_node_id}, depth={depth}, feature_rotated_axis={i}, "
                    f"threshold={thr:.4f}, impurity={node_impurity:.4f}")
    ...
    def _compute_reflection_and_split(self, X: np.ndarray, y: np.ndarray):
        ...
        Returns:
            Tuple[np.ndarray, Optional[Tuple[int, float]], Optional[Tuple[int, float]]]:
            Best reflection matrix, best split rule, and identity space rule (for comparison)
        """
        d = X.shape[1]
        ...
        if not self.force_oblique and rule_identity is not None:
            best_H = np.eye(d)
