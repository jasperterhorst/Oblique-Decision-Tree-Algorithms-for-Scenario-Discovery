...
def plot_tree_structure(hh, depth=None, coloring="distribution", color_axis_aligned=False, save=False,
        ...
        if isinstance(node, LeafNode):
            ...
            if node.is_axis_aligned:
                ...
                if len(idxs) == 1:
                    i = idxs[0]
                    coef = node.weights[i]
                    lefths = f"{coef:.2f}·{feature_names[i]}"
                else:
                ...
                for i, w in enumerate(node.weights):
                    if w != 0:
                        ...
                        terms.append(f"{sign}{w:.4f}·{feature_names[i]}")
                lefths = "<br/>".join(terms)
...
#             # === Format condition ===
#             if node.is_axis_aligned:
#                 idxs = np.flatnonzero(np.abs(node.weights) > 1e-4)
#                 if len(idxs) == 1:
#                     i = idxs[0]
#                     coef = node.weights[i]
#                     term = f"{coef:.2f}·{feature_names[i]}"
#                 else:
#                     term = "⚠ axis-aligned malformed"
#                 lefths = term
#             else:
#                 parts = []
#                 for i, w in enumerate(node.weights):
#                     if abs(w) > 1e-4:
#                         sign = "+" if w > 0 and parts else ""
#                         parts.append(f"{sign}{w:.2f}·{feature_names[i]}")
#                 lefths = "<br/>".join(parts)
#
#             label = (f"<<FONT POINT-SIZE='12'>{node_label_id}<br/>{lefths} ≤ {node.bias:.2f}"
#                      f"<br/>Samples: {sample_count}</FONT>>")
#
#             if color_axis_aligned and node.is_axis_aligned:
#                 color = "#ffcccc"
#             elif coloring == "class":
