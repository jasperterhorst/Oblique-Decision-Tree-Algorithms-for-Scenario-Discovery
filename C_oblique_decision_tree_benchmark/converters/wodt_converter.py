...
def convert_wodt(model):
    ...
    def recurse(node, node_id=0, depth=0):
        ...
        # Corrected: reconstruct full-dimensional weights
        weights = np.zeros(model.X.shape[1])
        for i, attr in enumerate(node.split.attrIDs):
            weights[attr] = node.split.paras[i]
