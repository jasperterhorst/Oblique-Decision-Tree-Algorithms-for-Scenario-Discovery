...
from D_oblique_decision_trees.evaluation.io_utils import get_output_dir
...
def plot_coverage_density_for_shape(df, algorithm=None, dataset=None, seed=None, print_points=False,
    ...
    cov = filtered["coverage"].values
    dens = filtered["density"].values
...
def plot_decision_regions_from_dict(X, y, trees_dict, max_depth, save_name=None, output_subfolder=None):
    ...
    ax = axes[0]
    scatter_colors = np.where(y == 0, SECONDARY_DARK, PRIMARY_DARK)
    ax.scatter(X[:, 0], X[:, 1], c=scatter_colors, edgecolor='k')
...
def plot_oblique_splits_from_dict(X, y, trees_dict, max_depth, save_name=None, output_subfolder=None):
    ...
    for depth in range(1, max_depth + 1):
        ...
        ax = axes[depth]
        ax.scatter(X[:, 0], X[:, 1], c=scatter_colors, s=20, alpha=0.6)
