...
class CO2Node:                                                      # defining the CO2 Node
    ...
    def __init__(self, depth, labels, **kwargs):
        ...
        self._prob_l = kwargs.get('pl', None)
        self._prob_r = kwargs.get('pr', None)
...
class ContinuouslyOptimizedObliqueTree(BaseEstimator):
    ...
    def __init__(self, impurity, segmentor, max_depth, min_samples_split=2, nu=1.0,
                 max_iter_per_node=10, tol=1e-3, eta=0.1, random_state=None):
    ...
    def objective(self, X, y, w, thetaL, thetaR):
        ...
        log_lik_l = self.likelihood_softmax(thetaL, index_y_samples)
        log_lik_r = self.likelihood_softmax(thetaR, index_y_samples)
        boundL = -np.dot(X, w) + log_lik_l
        boundR = np.dot(X, w) + log_lik_r
    ...
    def _generate_node(self, X, y, cur_depth):
        if self._terminate(X, y, cur_depth):
            ...
            while not converged:
                ...
                for t in range(1, (self.max_iter_per_node + 1)):
                    ...
                    for i in range(len(sample)):
                        ...
                        log_lik_l = self.likelihood_softmax(thetaL, index_y_samples[i])
                        log_lik_r = self.likelihood_softmax(thetaR, index_y_samples[i])
                        boundL = -np.dot(w, X[i, :]) + log_lik_l
                        boundR = np.dot(w, X[i, :]) + log_lik_r
    ...
    def get_params(self, deep=True):
        return {'max_depth': self._max_depth, 'min_samples_split': self._min_samples,
                'impurity': self.impurity, 'segmentor': self.segmentor, 'nu': self.nu,
                'max_iter_per_node': self.max_iter_per_node, 'tol': self.tol, 'eta': self.eta}
...
class CO2Classifier(ClassifierMixin, ContinuouslyOptimizedObliqueTree):
    def __init__(self, impurity, segmentor, max_depth=50, min_samples_split=2, nu=4.0, max_iter_per_node=10,
                 tol=1e-3, eta=0.01, random_state=None):
        super().__init__(impurity=impurity, segmentor=segmentor,
                         max_depth=max_depth, min_samples_split=min_samples_split,
                         nu=nu, max_iter_per_node=max_iter_per_node, tol=tol, eta=eta,
