...
def sum_impurity(left, right):
    """
    Compute the Sum of Impurity (SI) as used in OC1.
    Treats class labels numerically (e.g., 0, 1) and computes total variance from mean.
    """
    def impurity_side(y):
        if len(y) == 0:
            return 0.0
    ...
    return impurity_side(left) + impurity_side(right)
